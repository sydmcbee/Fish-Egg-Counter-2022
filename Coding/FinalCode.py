...
def fishImg(cam):
    ...
    imjTeim = ""
    TeimLbl = ["Yr", "Mnth", "Dy", "Hr", "Mnt", "Btc", "Cnt"]
    Numbatchstr = str(Numbatch)
    totcountstr = str(totcount)
    for n in range(0,5):
      imjTeim += TeimLbl[n]
      imjTeim += (str(time.gmtime()[n]))
      imjTeim += "-"
    imjTeim += TeimLbl[5]
    imjTeim += Numbatchstr
    imjTeim += "-"
    ...
    #cv2.imwrite('/media/pi/FilesOfFile/PtoSpm/{}.jpg'.format(imjTeim),im,[int(cv2.IMWRITE_JPEG_QUALITY), 50])
    #cv2.imwrite('/media/pi/FilesOfFile/PtoSpm/{:f}.jpg'.format(t1),im,[int(cv2.IMWRITE_JPEG_QUALITY), 50])
    cv2.imwrite('/home/pi/FtoSpm/{}.jpg'.format(imjTeim),im,[int(cv2.IMWRITE_JPEG_QUALITY), 50])
